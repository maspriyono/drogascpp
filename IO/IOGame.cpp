	...
	{		
		...
		while (i>=0){
			...
			while (j<=5){
				...
				input>>ch;
				//cout<<"id=";
				(*am).GetLFieldXY(j,i)->KerjaPaksa(atoi(ch)); //ini caranya cuk nanam
				//int as;
				//as=(((*am).GetLFieldXY(j,i))->GetPlant())->getID(); cout<<as;//apa aja di tanaman.h
				input>>ch; //cout<<ch;
				//cout<<"umur"
...
void IOGame::SaveFarmE(Player* org, waktu* tm, AllMap* am){
		...
		if (s.compare("")==0){ //berarti tidak ada di objek IO namafile-nya
			...
			if(tes){
				...
				cout<<"Ingin menindih yang lama(y/~))?";
				char p;
				cin>>p;
				if ((int(p)==89)||(int(p)==121)){
					SaveFarm(org, tm, am, name);
				}else{
...
void IOGame::OpenMap(AllMap* am){
	...
	{		
		...
		for (i=0; i<=5; i++){
			...
			h=ch[3];
			t=atoi(h.c_str());
			(*am).SetNLFieldXY(0,i,7,s,b,jp,t);
		}
		//something here
		input.close();
	}
